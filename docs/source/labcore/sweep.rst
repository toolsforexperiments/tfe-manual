...
Keywords specified in this way always override keywords that are passed around internally in the sweep (for more information see the :ref:`Passing Parameters in a Sweep` section):
...
.. note::
    ...
    For more information see: :ref:`Passing Parameters in a Sweep` section.
...
.. note::
    There are two different Sweep configuration related to passing arguments in Sweeps. For more information on them see the :ref:`Configuring Sweeps`.
...
A simple way of renaming conflicting arguments and **records** is to use the combination of ``lambda`` and :func:`record_as() <labcore.measurement.record.record_as>`:
...
:meth:`Sweep.record_none <labcore.measurement.sweep.Sweep.record_none>` , ``True`` by default, adds ``None`` to any **action** or **pointer** that didn't generate any **record** that iteration.
...
:class:`Sweep.pass_on_none <labcore.measurement.sweep.Sweep.pass_on_none>`, ``False`` by default, specifies if variables that return ``None`` should be passed as arguments to other **actions** or Sweeps (Because ``None`` is typically indicating that function did not return anything as data even though a record was declared using :class:`recording <labcore.measurement.record.recording>` or :func:`record_as() <labcore.measurement.record.record_as>`):
...
This is simple but does not do much else. If we only want to store the data generated by a Sweep in disk for later analysis we can use the function :func:`run_and_save_sweep() <labcore.ddh5.run_and_save_sweep>`:
...
.. note::
    :func:`run_and_save_sweep() <labcore.ddh5.run_and_save_sweep>` can save multiple objects to disk by accepting them as extra arguments.
...
If we also need this **action** to be a Sweep, the function :func:`once() <labcore.measurement.sweep.once>` will create a Sweep with no **pointer** that runs an **action** a single time:
